...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, num_tags=None, split_layer=5):
        self.inplanes = 64
        ...
        if num_tags is not None:
            if split_layer<=5:
                self.layer5_tag = self._make_pred_layer(Classifier_Module, [6,12,18,24],[6,12,18,24], num_tags)
            if split_layer<=4:
                self.layer4_tag = self._make_layer_given_inplane(block, 512, layers[3], stride=1, dilation=4, inplanes=1024)
    ...
    def _make_layer_given_inplane(self, block, planes, blocks, stride=1, dilation=1, inplanes=1024):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion or dilation == 2 or dilation == 4:
            downsample = nn.Sequential(
    ...
    def forward(self, x):
        ...
        seg = self.layer5(feat4)
        if self.num_tags is None:
            return seg
        if self.split_layer==5:
            tag = self.layer5_tag(feat4)
        elif self.split_layer==4:
            tag_ = self.layer4_tag(feat3)
            tag  = self.layer5_tag(tag_)
        return seg, tag
...
def Res_Deeplab(num_classes=21, num_tags=None, is_refine=False, split_layer=5):
    if is_refine:
        model = ResNet_Refine(Bottleneck,[3, 4, 23, 3], num_classes)
    else:
        model = ResNet(Bottleneck,[3, 4, 23, 3], num_classes, num_tags=num_tags, split_layer=split_layer)
    return model
